...
func main() {
	...
	items := append(getItems("https://wildberries.ru", "https://www.wildberries.ru/catalog/0/search.aspx?xsearch=true&search=" + s, params1), getItems("https://citilink.ru", "https://www.citilink.ru/search/?text=" + s, params2)...)
	encodeJSON(items)
}

func encodeJSON(items []*Item) {
	send, _ := json.Marshal(&templateJSON{len(items), items})
